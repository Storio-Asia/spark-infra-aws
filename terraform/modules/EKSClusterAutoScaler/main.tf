    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(var.eks_oidc_issuer_url, "https://", "")}:sub" = "system:serviceaccount:${kubernetes_service_account.metadata[0].namespace}:${kubernetes_service_account.metadata[0].name}"
